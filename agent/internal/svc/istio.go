...
func (istio *IstioService) ListGateway(namespace string, labelSelector string) (res []*v1beta1.Gateway, err error) {
	var conti string
	for {
		var listRes *v1beta1.GatewayList
		if listRes, err = istio.svcCtx.Istioclient.NetworkingV1beta1().Gateways(namespace).List(context.TODO(), metav1.ListOptions{
			LabelSelector: labelSelector,
			Continue:      conti,
			Limit:         10,
		}); err != nil {
		...
		res = append(res, processGatewayItems(listRes)...)
		if listRes.Continue == "" {
			break
		}
		conti = listRes.Continue
...
func (istio *IstioService) PatchGateway(namespace, name string, data []byte) (*v1beta1.Gateway, error) {
	istio.Logger.Infof("Patch gateway data: %s", string(data))
	return istio.svcCtx.Istioclient.NetworkingV1beta1().Gateways(namespace).Patch(istio.ctx, name, ktypes.MergePatchType, data, metav1.PatchOptions{})
}

func (istio *IstioService) GetGateway(namespace, name string) (res *v1beta1.Gateway, err error) {
	res, err = istio.svcCtx.Istioclient.NetworkingV1beta1().Gateways(namespace).Get(istio.ctx, name, metav1.GetOptions{})
	res.APIVersion = "networking.istio.io/v1beta1"
	res.Kind = "Gateway"
	res.UID = ""
	res.Generation = 0
...
func (istio *IstioService) DeleteGateway(namespace, name string) error {
	return istio.svcCtx.Istioclient.NetworkingV1beta1().Gateways(namespace).Delete(istio.ctx, name, metav1.DeleteOptions{})
}
...
func processDestinationItems(listRes *v1beta1.DestinationRuleList) []*v1beta1.DestinationRule {
	for i := range listRes.Items {
		...
		listRes.Items[i].Spec = istiometav1.DestinationRule{
			Host: listRes.Items[i].Spec.Host,
		}
...
func processVirtualItems(listRes *v1beta1.VirtualServiceList) []*v1beta1.VirtualService {
	for i := range listRes.Items {
		...
		listRes.Items[i].Spec = istiometav1.VirtualService{
			Hosts:    listRes.Items[i].Spec.Hosts,
			Gateways: listRes.Items[i].Spec.Gateways,
		}
		delete(listRes.Items[i].Annotations, "kubectl.kubernetes.io/last-applied-configuration")
	}
	return listRes.Items
...
func processGatewayItems(listRes *v1beta1.GatewayList) []*v1beta1.Gateway {
	for i := range listRes.Items {
		listRes.Items[i].ManagedFields = nil
		listRes.Items[i].Spec = istiometav1.Gateway{}
		delete(listRes.Items[i].Annotations, "kubectl.kubernetes.io/last-applied-configuration")
	}
	return listRes.Items
}
