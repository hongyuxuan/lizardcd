...
var setCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err := common.LizardServer.Patch(fmt.Sprintf("/lizardcd/cluster/%s/namespace/%s/statefulsets/%s?container=%s&image=%s", cluster, namespace, name, container, image)).Do(context.Background()).Err; err != nil {
