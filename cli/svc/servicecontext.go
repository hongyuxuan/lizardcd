...
func WritePodData(cluster, namespace, workloadType, workloadName string) (data [][]string) {
	...
	if err := common.LizardServer.Get(fmt.Sprintf("/lizardcd/cluster/%s/namespace/%s/%s/%s/pods", cluster, namespace, workloadType, workloadName)).SetResult(&res).Do(context.Background()).Err; err != nil {
...
func GetEvents(cluster, namespace, podName string) (data [][]string) {
	...
	if err := common.LizardServer.Get(fmt.Sprintf("/lizardcd/cluster/%s/namespace/%s/pods/%s/events", cluster, namespace, podName)).SetResult(&res).Do(context.Background()).Err; err != nil {
