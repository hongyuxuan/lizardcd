...
func NewHelmService(ctx context.Context) *HelmService {
	return &HelmService{
...
func (h *HelmService) SetKubeconfig(kubeconfig string) {
	h.kubeconfig = kubeconfig
}

func (h *HelmService) Update(repoList []*repo.Entry) error {
...
func (h *HelmService) SearchChart(repoUrl, repoName, chartName string) ([]*types.ChartListResponse, error) {
...
func (h *HelmService) SearchChartVersions(repoName, chartName string) ([]*types.ChartListResponse, error) {
	path := fmt.Sprintf("%s/%s-index.yaml", h.settings.RepositoryCache, repoName)
	indexFile, err := repo.LoadIndexFile(path)
	if err != nil {
...
func (h *HelmService) InstallChart(namespace, repoUrl, chartName, chartVersion, releaseName string, valuesYaml []byte, wait bool, timeout time.Duration) (err error) {
	actionConfig := new(action.Configuration)
	if h.kubeconfig != "" {
		kubeconfigPath := path.Join(os.TempDir(), namespace+"-kubeconfig")
		os.WriteFile(kubeconfigPath, []byte(h.kubeconfig), 0644)
		defer os.Remove(kubeconfigPath)
		h.settings.KubeConfig = kubeconfigPath
	}
...
func (h *HelmService) UninstallChart(namespace, releaseName string) (err error) {
	actionConfig := new(action.Configuration)
	if h.kubeconfig != "" {
		kubeconfigPath := path.Join(os.TempDir(), namespace+"-kubeconfig")
		os.WriteFile(kubeconfigPath, []byte(h.kubeconfig), 0644)
...
func (h *HelmService) UpgradeChart(namespace, repoUrl, releaseName, chartName, chartVersion string, revision int, valuesYaml []byte, wait bool, timeout time.Duration) (err error) {
	actionConfig := new(action.Configuration)
	if h.kubeconfig != "" {
		kubeconfigPath := path.Join(os.TempDir(), namespace+"-kubeconfig")
		os.WriteFile(kubeconfigPath, []byte(h.kubeconfig), 0644)
...
func (h *HelmService) ListRelease(namespace, releaseName string) (elements []types.ReleaseElement, err error) {
	actionConfig := new(action.Configuration)
	if h.kubeconfig != "" {
		kubeconfigPath := path.Join(os.TempDir(), namespace+"-kubeconfig")
		os.WriteFile(kubeconfigPath, []byte(h.kubeconfig), 0644)
...
func (h *HelmService) ShowDefaultValues(repoUrl, chartName, chartVersion string) (output string, err error) {
	actionConfig := new(action.Configuration)
...
func (h *HelmService) ShowReadme(repoUrl, chartName, chartVersion string) (output string, err error) {
	actionConfig := new(action.Configuration)
...
func (h *HelmService) GetValues(namespace, releaseName string, revision int) (output []byte, err error) {
	actionConfig := new(action.Configuration)
	if h.kubeconfig != "" {
		kubeconfigPath := path.Join(os.TempDir(), namespace+"-kubeconfig")
		os.WriteFile(kubeconfigPath, []byte(h.kubeconfig), 0644)
...
func (h *HelmService) ReleaseHistory(namespace, releaseName string) (res []types.ReleaseHistoryInfo, err error) {
	actionConfig := new(action.Configuration)
	if h.kubeconfig != "" {
		kubeconfigPath := path.Join(os.TempDir(), namespace+"-kubeconfig")
		os.WriteFile(kubeconfigPath, []byte(h.kubeconfig), 0644)
...
func (h *HelmService) Rollback(namespace, releaseName string, revision int, wait bool, timeout time.Duration) (err error) {
	actionConfig := new(action.Configuration)
	if h.kubeconfig != "" {
		kubeconfigPath := path.Join(os.TempDir(), namespace+"-kubeconfig")
		os.WriteFile(kubeconfigPath, []byte(h.kubeconfig), 0644)
...
func (h *HelmService) Pull(repoUrl, chartName, chartVersion, destDir string) (output string, err error) {
	actionConfig := new(action.Configuration)
