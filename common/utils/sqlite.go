...
func SetTx(tx *gorm.DB, req *commontypes.GetDataReq, count *int64, role string, tenant []string, jointTable *string) {
	if (req.Tablename == "application" ||
		req.Tablename == "image_repository" ||
		req.Tablename == "task_history" ||
		req.Tablename == "git_repository" ||
		req.Tablename == "ci_trigger" ||
		req.Tablename == "helm_repositories") && role != constant.ROLE_ADMIN {
		tx.Where(fmt.Sprintf("%s.tenant IN ?", req.Tablename), tenant)
	}
	if req.Search != "" {
