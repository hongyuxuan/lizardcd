...
>注意，您可能无法访问DockerHub，请将 `Image` 修改为能访问的镜像
...
## 发布版本
在当前页面，找到 `lizardtest` 应用，更多操作按钮选择`发布`，发布策略保持选择`所有工作负载使用相同镜像`。如果前面配置镜像仓库没有问题，选择制品这里会列出来所有的镜像 tags，选择一个tag。工作负载可以选择`是否启用`，这里保持启用，点击提交。
