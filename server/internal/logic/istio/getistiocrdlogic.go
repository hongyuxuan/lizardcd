...
func (l *GetIstioCrdLogic) GetIstioCrd(req *types.ResourceReq) (resp string, err error) {
	...
	} else if req.ResourceType == constant.ISTIO_CRD_TYPE_VIRTUALSERVICE {
	...
	} else if req.ResourceType == constant.ISTIO_CRD_TYPE_GATEWAY {
		if rpcResponse, err = ag.GetGateway(context.WithValue(l.ctx, commontypes.TraceIDKey{}, "rpc.GetGateway"), &agent.IstioGetRequest{
			Namespace: req.Namespace,
			Name:      req.ResourceName,
		}); err != nil {
			l.Logger.Error(err)
