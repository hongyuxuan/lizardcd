...
func (l *DeleteResourceLogic) DeleteResource(req *types.ResourceReq) (resp *types.Response, err error) {
	...
	if req.ResourceType == constant.ISTIO_CRD_TYPE_DESTINATIONRULE {
		if _, err = ag.DeleteDestinationRule(context.WithValue(l.ctx, "SpanName", "rpc.DeleteDestinationRule"), &agent.IstioGetRequest{
			Namespace: req.Namespace,
	...
	if req.ResourceType == constant.ISTIO_CRD_TYPE_VIRTUALSERVICE {
		if _, err = ag.DeleteVirtualService(context.WithValue(l.ctx, "SpanName", "rpc.DeleteVirtualService"), &agent.IstioGetRequest{
			Namespace: req.Namespace,
			Name:      req.ResourceName,
		}); err != nil {
			l.Logger.Error(err)
