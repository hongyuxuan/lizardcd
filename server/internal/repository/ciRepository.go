...
func (r *CiRepository) SaveCiTrigger(body map[string]interface{}) (id interface{}, err error) {
	...
	// save to database
	if err = r.svcCtx.Database.WithContext(context.WithValue(r.ctx, commontypes.TraceIDKey{}, "sqlite.CreateCiTrigger")).Save(&r.ciTrigger).Error; err != nil {
		return
	}
...
func (r *CiRepository) ListCiTrigger(req *commontypes.GetDataReq) (resp *types.Response, err error) {
	...
	tx := r.svcCtx.Database.WithContext(context.WithValue(r.ctx, commontypes.TraceIDKey{}, "sqlite.ListCiTrigger")).Model(commontypes.CiTrigger{})
...
func (r *CiRepository) SaveGitRepository(body map[string]interface{}) (id interface{}, err error) {
	...
	return r.gitRepo.Id, r.svcCtx.Database.WithContext(context.WithValue(r.ctx, commontypes.TraceIDKey{}, "sqlite.CreateGitRepository")).Save(&r.gitRepo).Error
}
...
func (r *CiRepository) ListGitRepository(req *commontypes.GetDataReq) (resp *types.Response, err error) {
	...
	var count int64
	tx := r.svcCtx.Database.WithContext(context.WithValue(r.ctx, commontypes.TraceIDKey{}, "sqlite.ListGitRepository")).Model(&commontypes.GitRepository{})
