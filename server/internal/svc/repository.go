...
func (r *RepoService) GetJrogArtifactList(repo commontypes.ImageRepository, repoName, imageName string) (files []commontypes.JfrogFileItem, err error) {
	...
	if err = client.SetBaseURL(repo.RepoUrl).
		Get(fmt.Sprintf("/artifactory/api/storage/%s/%s?list&deep=0&listFolders=1", repoName, imageName)).
		SetHeader("X-JFrog-Art-Api", repo.RepoPassword).
		SetSuccessResult(&resp).
		Do(context.WithValue(r.ctx, commontypes.TraceIDKey{}, "http.GetJrogArtifactList")).Err; err != nil {
		return
	}
...
func (r *RepoService) GetHarborArtifactList(repo commontypes.ImageRepository, repoName, imageName string) (files []commontypes.HarborFileItem, err error) {
	...
	if err = client.SetBaseURL(repo.RepoUrl).
		Get(fmt.Sprintf("/api/v2.0/projects/%s/repositories/%s/artifacts", repoName, imageName)).
		SetBasicAuth(repo.RepoAccount, repo.RepoPassword).
		SetSuccessResult(&files).
		Do(context.WithValue(r.ctx, commontypes.TraceIDKey{}, "http.GetHarborArtifactList")).Err; err != nil {
		return
	}
	return
}
...
func (r *RepoService) GetDockerHubImages(repo commontypes.ImageRepository, repoName, imageName, tag string) (files []commontypes.DockerHubImageItem, err error) {
	...
	if err = cli.Get(url).SetBearerAuthToken(repo.RepoPassword).SetSuccessResult(&resp).Do(context.WithValue(r.ctx, commontypes.TraceIDKey{}, "http.GetDockerHubImages")).Err; err != nil {
		return
	}
