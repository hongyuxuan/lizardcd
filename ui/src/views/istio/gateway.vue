    ...
    <el-row>
      ...
      <el-col :span="12">
        <el-button-group class="pull-right">
          <el-button class="pull-right" size="large" type="primary" @click="show.yaml=true;edit=false;yamlContent=''">+ 创建网关</el-button>
        </el-button-group>
      </el-col>
    </el-row>
    <el-table 
...
const getList = async (ifLoading) => {
  ...
  if(cluster.value !== "" && namespace.value !== "") {
    let response = await axios.get(`/lizardcd/istio/cluster/${cluster.value}/namespace/${namespace.value}/gateways`)
...
const editOne = async (row) => {
  ...
  edit.value = true
  yamlContent.value = await axios.get(`/lizardcd/istio/cluster/${cluster.value}/namespace/${namespace.value}/gateways/${row.metadata.name}/yaml`)
}
const deleteOne = async (row) => {
  await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/gateways/${row.metadata.name}`)
...
const submitYaml = async () => {
  ...
  await axios.patch(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/apply/yaml?kind=Gateway`, yamlContent.value, {
    headers: {
