...
<el-breadcrumb :separator-icon="ArrowRight">
  ...
  <el-breadcrumb-item>服务账户</el-breadcrumb-item>
</el-breadcrumb>
<div class="box box-solid">
  ...
  <div class="box-body" style="padding:0">
    <el-tabs v-model="activeName" type="border-card" class="qingcloud-tab">
      <el-tab-pane name="serviceaccounts" label="服务账户">
          ...
          <div class="box-body" style="padding-top:20px;padding-bottom:0">
            <el-row>
              ...
              <el-col :span="12">
                ...
                <el-button class="pull-right" size="large" type="primary" @click="show.new=true;">+ 新建服务账户</el-button>
              </el-col>
            </el-row>
            <el-table 
              :data="list" 
              ...
              <el-table-column label="" width="45">
                <font-awesome-icon icon="user" style="font-size:25px;vertical-align:middle;" />
              </el-table-column>
              <el-table-column prop="metadata.name" label="名称" min-width="200" />
              <el-table-column label="保密字典" min-width="250">
                <template #default="scope">
                  <el-link v-if="secrets[scope.row.metadata.name]" underline="never" :href="`/kubernetes/secrets/${secrets[scope.row.metadata.name]}?cluster=${cluster}&namespace=${namespace}`">{{ secrets[scope.row.metadata.name] }}</el-link>
                  <span v-else>-</span>
                </template>
              </el-table-column>
...
const activeName = ref("serviceaccounts")
const all = ref([])
const searchKey = ref("")
const list = ref([])
...
const getList = async (ifLoading) => {
  ...
  if(cluster.value && namespace.value) {
    let response = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/serviceaccounts`)
...
const getSecrets = async () => {
  let response = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/secrets?field_selector=type=kubernetes.io/service-account-token`)
  for(let x of response.results) {
    if(x.metadata.annotations?.hasOwnProperty("kubernetes.io/service-account.name")) {
      secrets.value[x.metadata.annotations["kubernetes.io/service-account.name"]] = x.metadata.name
    }
  }
}
const handleCommand = async (command) => {
  switch(command.action) {
      case "yaml": {
      yamlContent.value = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/serviceaccounts/${command.row.metadata.name}/yaml`)
    ...
    case "delete": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/serviceaccounts/${command.row.metadata.name}`)
...
const submitNew = async () => {
  refWorkload.value.getFormData(async (params) => {
    ...
    await axios.patch(`/lizardcd/kubernetes/cluster/${cluster}/namespace/${namespace}/apply/variable?kind=ServiceAccount`, params)
...
const handleMore = async (command) => {
  if(selected.value.length === 0) {
    ElMessage.warning({message: '请勾选服务账户'})
    return
  }
  switch(command.action) {
    case "deleteBatch": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        for(let x of selected.value) {
          await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/serviceaccounts/${x.metadata.name}`)
        }
        ElMessage.success({message: '删除成功'})
      }).catch(() =>{})
      break
