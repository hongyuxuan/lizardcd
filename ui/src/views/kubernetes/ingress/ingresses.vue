...
<el-breadcrumb :separator-icon="ArrowRight">
  ...
  <el-breadcrumb-item>路由</el-breadcrumb-item>
</el-breadcrumb>
<div class="box box-solid">
  <div class="box-header page-intro">
    <p>
      路由 (Ingress) 提供一种聚合服务的方式，您可以将集群的内部服务通过一个外部可访问的 IP 地址暴露给集群外部。
    </p>
  </div>
  <div class="box-body" style="padding:0">
    <el-tabs v-model="activeName" type="border-card" class="qingcloud-tab">
      <el-tab-pane name="ingresses" label="路由">
            ...
            <el-row>
              <el-col :span="16">
                <el-button-group>
                  ...
                  <el-input v-model="domainKey" placeholder="输入域名进行搜索" size="large" :prefix-icon="Search" @change="getPage(1)" clearable style="width:200px;" />
                </el-button-group>
              </el-col>
              <el-col :span="8">
                ...
                <el-button class="pull-right" size="large" type="primary" @click="show.new=true;">+ 新建路由</el-button>
              </el-col>
            </el-row>
            <el-table 
              :data="list" 
              ...
              <el-table-column label="" width="45">
                <font-awesome-icon icon="wifi" style="font-size:25px;vertical-align:middle;" />
              </el-table-column>
              <el-table-column prop="name" label="名称" min-width="200">
                <template #default="scope">
                  <el-link underline="never" :href="`/kubernetes/ingresses/${scope.row.metadata.name}?cluster=${cluster}&namespace=${namespace}`">{{ scope.row.metadata.name }}</el-link>
                </template>
              </el-table-column>
              <el-table-column prop="ip" label="网关地址" min-width="150">
                <template #default="scope">
                  <div v-if="Object.values(scope.row.status.loadBalancer).length>0">
                    {{ Object.values(scope.row.status.loadBalancer)[0][0].ip }}
                  </div>
                  <div v-else-if="scope.row.spec.type==='NodePort'">
                    {{ scope.row.spec.ports.map(x => `${x.targetPort}:${x.nodePort}/${x.protocol}`).join(";") }}
                  </div>
                  <div v-else>-</div>
                </template>
              </el-table-column>
...
const activeName = ref("ingresses")
const all = ref([])
const searchKey = ref("")
const domainKey = ref("")
const list = ref([])
...
const getList = async () => {
  ...
  if(cluster.value && namespace.value) {
    let response = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/ingresses`)
...
const getPage = async (page) => {
  ...
  if(searchKey.value !== '') {
    tmpList = tmpList.filter(n => n.metadata.name.includes(searchKey.value))
  }
  if(domainKey.value !== '') {
    tmpList = tmpList.filter(n => {
      let found = false
      for(let rule of n.spec.rules) {
        if(rule.host.includes(domainKey.value)) found = true
      }
      return found
    })
  }
...
const handleCommand = async (command) => {
  switch(command.action) {
      case "yaml": {
      yamlContent.value = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/ingresses/${command.row.metadata.name}/yaml`)
    ...
    case "delete": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/ingresses/${command.row.metadata.name}`)
...
const submitNew = async () => {
  refWorkload.value.getFormData(async (params) => {
    ...
    await axios.patch(`/lizardcd/kubernetes/cluster/${cluster}/namespace/${namespace}/apply/variable?kind=Ingress`, params)
    ...
    case "deleteBatch": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        for(let x of selected.value) {
          await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/ingresses/${x.metadata.name}`)
        }
        ElMessage.success({message: '删除成功'})
      }).catch(() =>{})
      break
