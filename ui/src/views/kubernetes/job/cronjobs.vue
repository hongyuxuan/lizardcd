      ...
      <el-col :span="12">
        <el-button-group>
          ...
          <el-select v-model="namespace" placeholder="请选择命名空间" clearable filterable @change="getList(true)" style="width:200px;" size="large">
            <el-option v-for="(item) in props.clusterList[cluster]" :key="item" :label="item" :value="item" />
          </el-select>
    ...
    <el-table 
      ...
      <el-table-column prop="name" label="名称" min-width="200">
        <template #default="scope">
          <el-link underline="never" :href="`/kubernetes/cronjobs/${scope.row.metadata.name}?cluster=${cluster}&namespace=${namespace}`">{{ scope.row.metadata.name }}</el-link>
        </template>
      </el-table-column>
      <el-table-column prop="ip" label="状态" min-width="150">
        <template #default="scope">
          <div v-if="scope.row.status.active||scope.row.status.lastScheduleTime"><font-awesome-icon icon="circle" class="text-success" style="font-size:12px" /> 运行中</div>
          <div v-else>Unknown</div>
        </template>
      </el-table-column>
      <el-table-column label="定时计划" width="120" prop="spec.schedule" />
      <el-table-column label="最近调度时间" width="170">
        <template #default="scope">
          {{ scope.row.status.lastScheduleTime?moment(scope.row.status.lastScheduleTime).format('YYYY-MM-DD HH:mm:ss'):'-' }}
        </template>
      </el-table-column>
      <el-table-column label="最近成功时间" width="170">
        <template #default="scope">
          {{ scope.row.status.lastSuccessfulTime?moment(scope.row.status.lastSuccessfulTime).format('YYYY-MM-DD HH:mm:ss'):'-' }}
        </template>
      </el-table-column>
...
const getList = async () => {
  ...
  if(cluster.value && namespace.value) {
    let response = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/cronjobs`)
...
const handleCommand = async (command) => {
  switch(command.action) {
     case "yaml": {
      yamlContent.value = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/cronjobs/${command.row.metadata.name}/yaml`)
    ...
    case "delete": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/cronjobs/${command.row.metadata.name}`)
    ...
    case "deleteBatch": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        for(let x of selected.value) {
          await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/cronjobs/${x.metadata.name}`)
        }
        ElMessage.success({message: '删除成功'})
      }).catch(() =>{})
      break
