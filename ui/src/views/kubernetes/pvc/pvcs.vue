...
<el-breadcrumb :separator-icon="ArrowRight">
  ...
  <el-breadcrumb-item>存储卷</el-breadcrumb-item>
</el-breadcrumb>
<div class="box box-solid">
  ...
  <div class="box-body" style="padding:0">
    <el-tabs v-model="activeName" type="border-card" class="qingcloud-tab">
      <el-tab-pane name="persistentvolumeclaims" label="存储卷">
          ...
          <div class="box-body" style="padding-top:20px;padding-bottom:0">
            <el-row>
              ...
              <el-col :span="12">
                ...
                <el-button class="pull-right" size="large" type="primary" @click="show.new=true;">+ 新建存储卷</el-button>
              </el-col>
            </el-row>
            <el-table 
              :data="list" 
              ...
              <el-table-column prop="name" label="名称" min-width="200">
                <template #default="scope">
                  <el-link underline="never" :href="`/kubernetes/persistentvolumeclaims/${scope.row.metadata.name}?cluster=${cluster}&namespace=${namespace}`">{{ scope.row.metadata.name }}</el-link>
                  <div class="text-gray cell-comment" style="line-height:18px;padding-top:0;">{{scope.row.spec.storageClassName||'-'}}</div>
                </template>
              </el-table-column>
              <el-table-column label="状态" width="100">
                <template #default="scope">
                  {{ scope.row.status.phase || 'UnBound' }}
                </template>
              </el-table-column>
              <el-table-column label="访问模式" width="150">
                <template #default="scope">
                  {{ scope.row.spec.accessModes.join(",") }}
                </template>
              </el-table-column>
...
const activeName = ref("persistentvolumeclaims")
const all = ref([])
const searchKey = ref("")
const list = ref([])
...
const getList = async (ifLoading) => {
  ...
  if(cluster.value && namespace.value) {
    let response = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/persistentvolumeclaims`)
...
const handleCommand = async (command) => {
  switch(command.action) {
     case "yaml": {
      yamlContent.value = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/persistentvolumeclaims/${command.row.metadata.name}/yaml`)
    ...
    case "delete": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/persistentvolumeclaims/${command.row.metadata.name}`)
...
const submitNew = async () => {
  refWorkload.value.getFormData(async (params) => {
    ...
    await axios.patch(`/lizardcd/kubernetes/cluster/${cluster}/namespace/${namespace}/apply/variable?kind=PersistentVolumeClaim`, params)
...
const handleMore = async (command) => {
  if(selected.value.length === 0) {
    ElMessage.warning({message: '请勾选存储卷'})
    return
  }
  switch(command.action) {
    case "deleteBatch": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        for(let x of selected.value) {
          await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/persistentvolumeclaims/${x.metadata.name}`)
        }
        ElMessage.success({message: '删除成功'})
      }).catch(() =>{})
      break
