...
<div class="box box-solid">
  ...
  <div class="box-body" style="padding:0">
    <el-tabs v-model="activeName" type="border-card" class="qingcloud-tab">
      <el-tab-pane name="services" label="服务">
          ...
          <div class="box-body" style="padding-top:20px;padding-bottom:0">
            <el-row>
              ...
              <el-col :span="12">
                ...
                <el-button class="pull-right" size="large" type="primary" @click="show.new=true;">+ 新建服务</el-button>
              </el-col>
            </el-row>
            <el-table 
              :data="list" 
              ...
              <el-table-column label="" width="45">
                <font-awesome-icon icon="sitemap" style="font-size:25px;vertical-align:middle;" />
              </el-table-column>
              <el-table-column prop="name" label="名称" min-width="200">
                <template #default="scope">
                  <el-link underline="never" :href="`/kubernetes/services/${scope.row.metadata.name}?cluster=${cluster}&namespace=${namespace}`">{{ scope.row.metadata.name }}</el-link>
                </template>
              </el-table-column>
              <el-table-column prop="spec.type" label="服务类型" min-width="120" />
              <el-table-column prop="ip" label="内部访问" min-width="150">
                <template #default="scope">
                  <div>{{ scope.row.spec.clusterIP }}</div>
                  <div class="text-gray cell-comment" style="line-height:18px;padding-top:0;">{{ scope.row.spec.clusterIP==='None'?'Headless':'VirtualIP' }}</div>
                </template>
              </el-table-column>
              <el-table-column prop="ip" label="外部访问" min-width="150">
                <template #default="scope">
...
const getList = async () => {
  ...
  if(cluster.value && namespace.value) {
    let response = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/services`)
...
const handleCommand = async (command) => {
  switch(command.action) {
     case "yaml": {
      yamlContent.value = await axios.get(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/services/${command.row.metadata.name}/yaml`)
    ...
    case "delete": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/services/${command.row.metadata.name}`)
...
const submitNew = async () => {
  refWorkload.value.getFormData(async (params) => {
    ...
    await axios.patch(`/lizardcd/kubernetes/cluster/${cluster}/namespace/${namespace}/apply/variable?kind=Service`, params)
...
const handleMore = async (command) => {
  if(selected.value.length === 0) {
    ElMessage.warning({message: '请勾选服务'})
    return
  }
  switch(command.action) {
    case "deleteBatch": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        for(let x of selected.value) {
          await axios.delete(`/lizardcd/kubernetes/cluster/${cluster.value}/namespace/${namespace.value}/services/${x.metadata.name}`)
        }
        ElMessage.success({message: '删除成功'})
      }).catch(() =>{})
      break
