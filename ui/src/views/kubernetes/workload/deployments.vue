  ...
  <div class="box-body" style="padding-top:20px;padding-bottom:0">
    <el-row>
      ...
      <el-col :span="12">
        ...
        <el-button class="pull-right" size="large" type="primary" @click="show.new=true;">+ 新建工作负载</el-button>
      </el-col>
    </el-row>
    <el-table 
      :data="list" 
      ...
      <el-table-column prop="name" label="名称" min-width="200">
        <template #default="scope">
          <el-link underline="never" :href="`/kubernetes/workload/deployments/${scope.row.name}?cluster=${cluster}&namespace=${namespace}`">{{ scope.row.name }}</el-link>
        </template>
      </el-table-column>
...
const getList = async (ifLoading) => {
  ...
  if(cluster.value && namespace.value) {
    ...
    response = response.results.map(x => {
...
const handleCommand = async (command) => {
  switch(command.action) {
    ...
    case "setImage": {
      ...
      let image = pods[0].spec.containers[0].image
...
const submitNew = async () => {
  refWorkload.value.getFormData(async (params) => {
    ...
    await axios.patch(`/lizardcd/kubernetes/cluster/${cluster}/namespace/${namespace}/apply/variable?kind=Deployment`, params)
