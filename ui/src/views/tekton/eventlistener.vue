...
<div class="box box-item">
  <div class="box-body" style="padding-top:20px;padding-bottom:0">
    <el-alert type="info" show-icon style="margin-bottom:15px">
      <template #title>
        关于 EventListener 配置参考：<el-link href="https://tekton.dev/docs/triggers/eventlisteners/" underline="never" type="primary" target="_blank">EventListeners</el-link>
      </template>
    </el-alert>
    <el-row>
      <el-col :span="18">
...
const getList = async (ifLoading) => {
  ...
  if(props.defaultTekton.cluster !== "" && namespace.value !== "") {
    let response = await axios.get(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/eventlisteners`)
...
const editOne = async (row) => {
  ...
  edit.value = true
  form.value.content = await axios.get(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/eventlisteners/${row.metadata.name}/yaml`)
}
const deleteOne = async (row) => {
  await axios.delete(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/eventlisteners/${row.metadata.name}`)
...
const submitYaml = async () => {
  ...
  try {
    if(edit.value === false) {
      await axios.post(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/apply?kind=EventListener`, params)
