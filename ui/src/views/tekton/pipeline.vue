...
<div class="box box-item">
  <div class="box-body" style="padding-top:20px;padding-bottom:0">
    <el-alert type="warning" show-icon style="margin-bottom:15px">
      <template #title>
        关于 Pipeline 配置参考：<el-link href="https://tekton.dev/docs/pipelines/pipelines/" :underline="false" type="primary" target="_blank">Pipelines</el-link>
      </template>
    </el-alert>
    <el-row>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-button-group class="pull-right">
          <el-button class="pull-right" size="large" type="primary" @click="show.yaml=true;edit=false;form={content:'',variables:[]}">+ 创建Pipeline</el-button>
        </el-button-group>
      </el-col>
    </el-row>
    <el-table 
...
const getList = async (ifLoading) => {
  ...
  if(cluster.value !== "" && namespace.value !== "") {
    let response = await axios.get(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/pipelines`)
...
const editOne = async (row) => {
  ...
  edit.value = true
  form.value.content = await axios.get(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/pipelines/${row.metadata.name}/yaml`)
}
const deleteOne = async (row) => {
  await axios.delete(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/pipelines/${row.metadata.name}`)
...
const submitYaml = async () => {
  ...
  if(edit.value === false) {
    await axios.post(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/apply?kind=Pipeline`, params)
