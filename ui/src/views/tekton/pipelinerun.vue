...
<div class="box box-item">
  <div class="box-body" style="padding-top:20px;padding-bottom:0">
    <el-alert type="warning" show-icon style="margin-bottom:15px">
      <template #title>
        关于 pipelineRun 配置参考：<el-link href="https://tekton.dev/docs/pipelines/pipelineruns/" :underline="false" type="primary" target="_blank">PipelineRuns</el-link>
      </template>
    </el-alert>
    <el-row>
      <el-col :span="12">
        <el-button-group>
          ...
          <el-select v-model="namespace" placeholder="请选择命名空间" clearable filterable @change="getTektonEndpoint();getList(true)" style="width:200px;margin-right:5px" size="large">
      ...
      <el-col :span="12">
        ...
        <el-button class="pull-right" size="large" type="primary" @click="show.yaml=true;edit=false;form={content:'',variables:[]}" style="margin-right:5px">+ 创建PipelineRun</el-button>
      </el-col>
    </el-row>
    <el-table 
      :data="list" 
      ...
      <el-table-column prop="metadata.name" label="名称" min-width="160">
        <template #default="scope">
          <el-link :underline="false" :href="`${tektonEndpoint}/#/namespaces/${namespace}/pipelineruns/${scope.row.metadata.name}`" target="_blank">{{ scope.row.metadata.name }}</el-link>
        </template>
      </el-table-column>
      <el-table-column prop="spec.pipelineRef.name" label="关联流水线" min-width="160" />
      <el-table-column label="执行状态" width="120">
        <template #default="scope">
          <el-progress v-if="scope.row.status?.conditions[0].status=='True'" :percentage="100" color="#5cb87a" :show-text="false" />
          <el-tooltip v-else-if="scope.row.status?.conditions[0].status=='False'" effect="dark" placement="top" :content="scope.row.status.conditions[0].message">
...
/* methods */
const getTektonEndpoint = async () => {
  tektonEndpoint.value = await axios.get(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/endpoint`)
}
...
const getList = async (ifLoading) => {
  ...
  if(cluster.value !== "" && namespace.value !== "") {
    let response = await axios.get(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/pipelineruns`)
...
const editOne = async (row) => {
  ...
  edit.value = true
  form.value.content = await axios.get(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/pipelineruns/${row.metadata.name}/yaml`)
}
const deleteOne = async (row) => {
  await axios.delete(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/pipelineruns/${row.metadata.name}`)
...
const submitYaml = async () => {
  ...
  if(edit.value === false) {
    await axios.post(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/apply?kind=PipelineRun`, params)
...
const reRun = async (row) => {
  ...
  await axios.post(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/apply?kind=PipelineRun`, {content: yaml.dump(params)})
    ...
    case "deleteBatch": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        await Promise.all(selected.value.map(x => {
          return axios.delete(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/pipelineruns/${x.metadata.name}`)
        }))
        ElMessage.success({message: `删除PipelineRun成功`})
        setTimeout(async () => {
          await getList(true)
