...
<div class="box box-item">
  <div class="box-body" style="padding-top:20px;padding-bottom:0">
    ...
    <el-table 
      ...
      <el-table-column prop="metadata.name" label="名称" min-width="160">
        <template #default="scope">
          <el-link underline="never" :href="`/ci/tekton/pipelinerun/${scope.row.metadata.name}?cluster=${defaultTekton.cluster}&namespace=${namespace}`" target="_blank">{{ scope.row.metadata.name }}</el-link>
        </template>
      </el-table-column>
      <el-table-column prop="spec.pipelineRef.name" label="关联流水线" min-width="160" />
      <el-table-column label="执行状态" width="140">
        <template #default="scope">
          <el-tooltip v-if="scope.row.spec?.status" placement="top" :content="scope.row.spec.status">
            <el-progress  :percentage="100" color="#bfbbbb" :show-text="false" />
          </el-tooltip>
          <el-progress v-else-if="scope.row.status?.conditions[0].reason=='Succeeded'" :percentage="100" color="#5cb87a" :show-text="false" />
...
const getList = async (ifLoading) => {
  ...
  if(props.defaultTekton.cluster !== "" && namespace.value !== "") {
    let response = await axios.get(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/pipelineruns`)
...
const editOne = async (row) => {
  ...
  edit.value = true
  form.value.content = await axios.get(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/pipelineruns/${row.metadata.name}/yaml`)
}
const deleteOne = async (row) => {
  await axios.delete(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/pipelineruns/${row.metadata.name}`)
...
const reRun = async (row) => {
  ...
  params.metadata.generateName ||= params.spec.pipelineRef.name + "-"
  ...
  switch(command.action) {
    case "deleteBatch": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        await Promise.all(selected.value.map(x => {
          return axios.delete(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/pipelineruns/${x.metadata.name}`)
        }))
        ElMessage.success({message: `删除PipelineRun成功`})
        setTimeout(async () => {
          await getList(true)
    ...
    case "cancelBatch": {
      await ElMessageBox.confirm('确定终止？','警告', {
        ...
        await Promise.all(selected.value.map(x => {
          return axios.patch(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/pipelineruns/${x.metadata.name}`, [
            {
              "op": "replace",
              "path": "/spec/status",
              "value": "Cancelled"
...
const cancel = async (row) => {
  await axios.patch(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/pipelineruns/${row.metadata.name}`, [
    {
      "op": "replace",
      "path": "/spec/status",
      "value": "Cancelled"
