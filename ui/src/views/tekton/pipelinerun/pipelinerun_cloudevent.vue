...
<div class="box box-item">
  <div class="box-body" style="padding-top:20px;padding-bottom:0">
    <el-alert type="info" show-icon style="margin-bottom:15px">
      <template #title>
        关于 pipelineRun 配置参考：<el-link href="https://tekton.dev/docs/pipelines/pipelineruns/" underline="never" type="primary" target="_blank">PipelineRuns</el-link>
      </template>
    </el-alert>
    <el-row>
      <el-col :span="18">
        <el-button-group>
          ...
          <el-select v-model="namespace" placeholder="请选择命名空间" clearable filterable @change="getList(1);current=1;getPipelines()" style="width:250px;" size="large">
            <el-option v-for="(item) in props.namespaceList" :key="item" :label="item" :value="item" />
          </el-select>
          <el-select v-model="pipelineName" placeholder="过滤流水线" clearable filterable @change="getList(1);current=1;" style="width:250px;" size="large">
            <el-option v-for="(item) in pipelineList" :key="item" :label="item" :value="item" />
          </el-select>
          <el-input v-model="searchKey" placeholder="输入名称进行搜索" size="large" :prefix-icon="Search" @change="getList(1);current=1" clearable style="width:250px;" />
        </el-button-group>
      </el-col>
      <el-col :span="6">
        <el-dropdown @command="handleMore" class="pull-right">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <el-dropdown-item :command="{action:'cancelBatch'}">批量终止</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
        <el-button class="pull-right" size="large" type="primary" @click="show.yaml=true;edit=false;form={content:'',variables:[]}">+ 创建PipelineRun</el-button>
      </el-col>
    </el-row>
    <el-table 
      :data="list" 
      ...
      <el-table-column prop="name" label="名称" min-width="160">
        <template #default="scope">
          <el-link underline="never" :href="`/ci/tekton/pipelinerun/${scope.row.name}?cluster=${defaultTekton.cluster}&namespace=${namespace}`" target="_blank">{{ scope.row.name }}</el-link>
        </template>
      </el-table-column>
      <el-table-column prop="pipelineRef" label="关联流水线" min-width="160" />
      <el-table-column label="执行状态" width="140">
        <template #default="scope">
          <!-- <el-link underline="never" :href="`/ci/tekton/pipelinerun/${scope.row.name}?cluster=${defaultTekton.cluster}&namespace=${namespace}`" target="_blank"> -->
            <el-progress v-if="scope.row.conditions[0].reason=='Succeeded'" :percentage="100" color="#5cb87a" :show-text="false" />
            <el-tooltip v-else-if="scope.row.conditions[0].reason=='Cancelled'" effect="dark" placement="top" content="Cancelled">
              <el-progress :percentage="100" color="#bfbbbb" :show-text="false" />
            </el-tooltip>
            <el-progress v-else-if="scope.row.conditions[0].status=='Unknown'" :percentage="50" color="#e6a23c" :show-text="false" />
            <el-tooltip v-else effect="dark" placement="top" :content="scope.row.conditions[0].message">
      ...
      <el-table-column label="耗时" width="120">
        <template #default="scope">
          <span v-if="scope.row.endTime&&scope.row.startTime">
            {{ moment.duration(moment(scope.row.endTime)-moment(scope.row.startTime)).humanize() }}
          </span>
        </template>
      </el-table-column>
...
<el-drawer v-model="show.yaml" direction="rtl" size="700px">
  <template #header>
    <h4>{{ edit === true ? '编辑YAML' : '创建pipelineRun' }}</h4>
  </template>
  <template #default>
    <el-form ref="task" label-width="80px">
...
onMounted(async () => {
  if(route.query.namespace) {
    ...
    await getPipelines()
    setTimeout(async () => {
      pipelineName.value = route.query.pipelineName || ""
      await getList(1)
    }, 500)
  }
})
...
const getPipelines = async () => {
  if(props.defaultTekton.cluster !== "" && namespace.value !== "") {
    ...
    if(role.value !== 'admin') {
      url += `?label_selector=project in (${localStorage.tenant})`
    }
    let response = await axios.get(url)
    pipelineList.value = response.results.map(x => x.metadata.name)
  }
}
const getList = async (page) => {
  loading.value = true
  if(namespace.value !== "") {
    let url = `/tekton-pipelines/lizardcd/${namespace.value}/pipelineRuns?page=${page}&size=${pageSize.value}&orderBy=startTime&sort=DESC`
    if(searchKey.value !== '') {
      url += `&name=${searchKey.value}`
    }
    if(pipelineName.value) {
      url += `&pipelineRef=${pipelineName.value}`
    }
    let response = await axios.get(url)
...
const getTemlates = async () => {
  let response = await axios.get(`/lizardcd/db/yaml_template?filter=type==tekton_pipelinerun&page=1&size=100&sort=update_at desc`)
...
const editOne = async (row) => {
  ...
  edit.value = true
  let response = await axios.get(`/tekton-pipelines/lizardcd/${namespace.value}/pipelineRuns/${row.name}/yaml`)
  ...
  form.value.content = yaml.dump(o)
}
const deleteOne = async (row) => {
  await axios.delete(`/tekton-pipelines/lizardcd/${namespace.value}/pipelineRuns/${row.name}`)
...
const submitYaml = async () => {
  ...
  try {
    if(edit.value === false) {
      await axios.post(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/apply?kind=PipelineRun`, params)
      ElMessage.success({message: `创建PipelineRun成功`})
    }
    else {
      delete params.variables
      await axios.post(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/apply?kind=PipelineRun`, params)
      ElMessage.success({message: `更新PipelineRun成功`})
    }
...
const reRun = async (row) => {
  let content = await axios.get(`/tekton-pipelines/lizardcd/${namespace.value}/pipelineRuns/${row.name}/yaml`)
  let params = yaml.load(content)
  params.metadata.generateName ||= row.pipelineRef + "-"
  delete params.metadata.name
  delete params.spec.status
  try {
    await axios.post(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/apply?kind=PipelineRun`, {content: yaml.dump(params)})
...
const handleMore = async (command) => {
  if(selected.value.length === 0) {
    ElMessage.warning({message: '请勾选PipelineRun'})
    return
  }
  switch(command.action) {
    case "deleteBatch": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        await Promise.all(selected.value.map(x => {
          return axios.delete(`/tekton-pipelines/lizardcd/${namespace.value}/pipelineRuns/${x.name}`)
        }))
        ElMessage.success({message: `删除PipelineRun成功`})
        setTimeout(async () => {
          await getList(current.value)
    ...
    case "cancelBatch": {
      await ElMessageBox.confirm('确定终止？','警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(async () => {
        await Promise.all(selected.value.map(x => {
          return axios.patch(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/pipelineruns/${x.name}`, [
            {
              "op": "replace",
              "path": "/spec/status",
              "value": "Cancelled"
...
const cancel = async (row) => {
  await axios.patch(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/pipelineruns/${row.name}`, [
    {
      "op": "replace",
      "path": "/spec/status",
      "value": "Cancelled"
