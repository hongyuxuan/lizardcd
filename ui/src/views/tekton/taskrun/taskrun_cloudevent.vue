    ...
    <el-row>
      <el-col :span="18">
        <el-button-group>
          ...
          <el-select v-model="namespace" placeholder="请选择命名空间" clearable filterable @change="getList(1);current=1" style="width:250px;" size="large">
    ...
    <el-table 
      ...
      <el-table-column label="名称" min-width="200">
        <template #default="scope">
          <el-link v-if="scope.row.taskKind !== 'ApprovalTask'" underline="never" :href="`/ci/tekton/taskrun/${scope.row.name}?cluster=${defaultTekton.cluster}&namespace=${namespace}`" target="_blank">{{ scope.row.name }}</el-link>
      ...
      <el-table-column label="执行状态" width="120">
        <template #default="scope">
          ...
          <el-tooltip v-else-if="scope.row.conditions[0]?.reason=='RunTimedOut'" effect="dark" placement="top" :content="scope.row.conditions[0]?.message">
            <el-progress :percentage="100" color="#f56c6c" :show-text="false" />
          </el-tooltip>
          <el-tooltip v-else-if="scope.row.conditions[0]?.reason=='TaskRunCancelled'" effect="dark" placement="top" content="TaskRunCancelled">
          ...
          <!-- approvaltask -->
          <el-progress v-else-if="scope.row.approvalStatus?.status=='pending'" :percentage="50" color="#e6a23c" :show-text="false" />
          <!-- approvetsk end -->
...
const getList = async (page) => {
  loading.value = true
  if(namespace.value !== "") {
    let url = `/tekton-pipelines/lizardcd/${namespace.value}/taskRuns?page=${page}&size=${pageSize.value}&orderBy=startTime&sort=DESC`
    if(searchKey.value !== '') {
...
const editOne = async (row) => {
  ...
  edit.value = true
  if(row.taskKind === 'ApprovalTask') {
    form.value.content = yaml.dump(row)
  } 
  else {
    let response = await axios.get(`/tekton-pipelines/lizardcd/${namespace.value}/taskRuns/${row.name}/yaml`)
    let o = yaml.load(response)
    if(o.spec?.timeout) {
      let timeout = parseInt(o.spec.timeout.slice(0, -2))
      const duration = Duration.fromMillis(timeout / 1e6)
      o.spec.timeout = duration.toFormat("h'h'm'm's's'")
    }
    delete o.status
...
const deleteOne = async (row) => {
  await axios.delete(`/tekton-pipelines/lizardcd/${namespace.value}/taskRuns/${row.name}`)
...
const reRun = async (row) => {
  let content = await axios.get(`/tekton-pipelines/lizardcd/${namespace.value}/taskRuns/${row.name}/yaml`)
  let params = yaml.load(content)
  let arr = row.taskRef.split("/")
  if(arr.length > 1) {
    params.metadata.generateName ||= arr[1] + "-"
  } else {
    params.metadata.generateName ||= params.metadata.name + "-"
  }
    ...
    case "deleteBatch": {
      await ElMessageBox.confirm('确定删除？','警告', {
        ...
        await Promise.all(selected.value.map(x => {
          return axios.delete(`/tekton-pipelines/lizardcd/${namespace.value}/taskRuns/${x.name}`)
        }))
        ElMessage.success({message: `删除TaskRun成功`})
        setTimeout(async () => {
          await getList(current.value)
...
const cancel = async (row) => {
  try {
    await axios.patch(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/taskruns/${row.name}`, [
      {
        "op": "replace",
        "path": "/spec/status",
