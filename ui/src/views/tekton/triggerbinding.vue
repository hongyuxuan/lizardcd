...
<el-drawer v-model="show.yaml" direction="rtl" size="700px">
  <template #header>
    <h4>{{ edit === true ? '编辑YAML' : '创建TriggerBinding' }}</h4>
  </template>
  <template #default>
...
const getTemlates = async () => {
  let response = await axios.get(`/lizardcd/db/yaml_template?filter=type==tekton_triggerbinding&page=1&size=100&sort=update_at desc`)
