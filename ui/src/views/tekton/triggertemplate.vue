...
<div class="box box-item">
  <div class="box-body" style="padding-top:20px;padding-bottom:0">
    <el-alert type="info" show-icon style="margin-bottom:15px">
      <template #title>
        关于 TriggerTemplate 配置参考：<el-link href="https://tekton.dev/docs/triggers/triggertemplates/" underline="never" type="primary" target="_blank">TriggerTemplates</el-link>
      </template>
    </el-alert>
    <el-row>
      <el-col :span="18">
      ...
      <el-col :span="6">
        <el-button-group class="pull-right">
          <el-button class="pull-right" size="large" type="primary" @click="show.yaml=true;edit=false;form={content:'',variables:[]}">+ 创建TriggerTemplate</el-button>
        </el-button-group>
      </el-col>
    </el-row>
    <el-table 
...
<el-drawer v-model="show.yaml" direction="rtl" size="700px">
  <template #header>
    <h4>{{ edit === true ? '编辑YAML' : '创建TriggerTemplate' }}</h4>
  </template>
  <template #default>
...
const getList = async (ifLoading) => {
  ...
  if(props.defaultTekton.cluster !== "" && namespace.value !== "") {
    let response = await axios.get(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/triggertemplates`)
...
const getTemlates = async () => {
  let response = await axios.get(`/lizardcd/db/yaml_template?filter=type==tekton_triggertemplate&page=1&size=100&sort=update_at desc`)
...
const editOne = async (row) => {
  ...
  edit.value = true
  form.value.content = await axios.get(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/triggertemplates/${row.metadata.name}/yaml`)
}
const deleteOne = async (row) => {
  await axios.delete(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/triggertemplates/${row.metadata.name}`)
  ElMessage.success({message: `删除TriggerTemplate成功`})
  setTimeout(async () => {
    await getList(true)
  }, 500)
}
const submitYaml = async () => {
  ...
  try {
    if(edit.value === false) {
      await axios.post(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/apply?kind=TriggerTemplate`, params)
      ElMessage.success({message: `创建TriggerTemplate成功`})
    }
    else {
      delete params.variables
      await axios.post(`/lizardcd/tekton/cluster/${props.defaultTekton.cluster}/namespace/${namespace.value}/apply?kind=TriggerTemplate`, params)
      ElMessage.success({message: `更新TriggerTemplate成功`})
    }
