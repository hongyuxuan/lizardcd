...
<div class="box box-item">
  <div class="box-body" style="padding-top:20px;padding-bottom:0">
    <el-alert type="warning" show-icon style="margin-bottom:15px">
      <template #title>
        关于 TriggerTemplate 配置参考：<el-link href="https://tekton.dev/docs/triggers/triggertemplates/" :underline="false" type="primary" target="_blank">TriggerTemplates</el-link>
      </template>
    </el-alert>
    <el-row>
      <el-col :span="12">
...
const getList = async (ifLoading) => {
  ...
  if(cluster.value !== "" && namespace.value !== "") {
    let response = await axios.get(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/triggertemplates`)
...
const editOne = async (row) => {
  ...
  edit.value = true
  form.value.content = await axios.get(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/triggertemplates/${row.metadata.name}/yaml`)
}
const deleteOne = async (row) => {
  await axios.delete(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/triggertemplates/${row.metadata.name}`)
...
const submitYaml = async () => {
  ...
  if(edit.value === false) {
    await axios.post(`/lizardcd/tekton/cluster/${cluster.value}/namespace/${namespace.value}/apply?kind=TriggerTemplate`, params)
