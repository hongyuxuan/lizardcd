...
<el-card>
  <template #header>
    <div class="card-header">
      <span class="card-header-text">模板管理</span>
    </div>
  </template>
  <el-row>
  ...
  <el-table 
    ...
    <el-table-column prop="Option" label="操作" width="140">
      <template #default="scope">
        <el-button :icon="EditPen" circle @click="editOne(scope.row)" :disabled="scope.row.tenant!==tenant" />
        <el-tooltip effect="dark" content="复制" placement="top">
          <el-button :icon="CopyDocument" circle @click="copyOne(scope.row)" />
        </el-tooltip>
        <el-popconfirm title="确认删除？" @confirm="deleteOne(scope.row)">
          <template #reference>
            <el-button :icon="Delete" circle :disabled="scope.row.tenant!==tenant" />
          </template>
        </el-popconfirm>
      </template>
    </el-table-column>
...
<el-drawer v-model="show" direction="rtl" size="700px">
  ...
  <template #default>
    <el-form ref="template" :model="form" :rules="rules" label-width="120px">
      ...
      <el-form-item label="模板类型" prop="name">
        <el-input v-model="form.type" size="large" />
      </el-form-item>
  ...
  <template #footer>
    <div style="flex: auto">
      ...
      <el-button type="primary" @click="confirmClick(template)">提交</el-button>
    </div>
  </template>
</el-drawer>
</template>
...
const getList = async (page) => {
  ...
  let response = await axios.get(`/lizardcd/db/yaml_template?${url}`)
...
const confirmClick = async (f) => {
  ...
  await f.validate(async (valid) => {
    if(valid) {
      ...
      if(edit.value === false) {
        await axios.post(`/lizardcd/db/yaml_template`, {body:params})
        getList(1)
        current.value = 1
        show.value = false
      }
        ...
        let id = params.id
        delete params.id
        await axios.put(`/lizardcd/db/yaml_template/${id}`, {body:params})
...
const deleteOne = async (row) => {
  await axios.delete(`/lizardcd/db/yaml_template/${row.id}`)
  getList(current.value)
}
