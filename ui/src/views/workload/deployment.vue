...
const submitPatch = async (f) => {
  ...
  await f.validate(async (valid) => {
    if(valid) {
      await axios.patch(`/kubernetes/cluster/${form.value.cluster}/namespace/${form.value.namespace}/deployments/${form.value.workload_name}?container=${form.value.container}&image=${form.value.image}`)
