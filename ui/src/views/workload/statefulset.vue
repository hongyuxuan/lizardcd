...
<el-card>
  ...
  <el-row>
    ...
    <el-col :span="12">
      <el-button class="pull-right" size="large" type="primary" @click="show.add=true;form={}">更新镜像</el-button>
      <el-button class="pull-right" size="large" type="primary" @click="show.new=true;formNew={content:'',variables:[]}" style="margin-right:5px">新建发布</el-button>
    </el-col>
  </el-row>
  <el-table 
    :data="list" 
...
<el-drawer v-model="show.add" direction="rtl" size="600px">
  ...
  <template #default>
    <el-form ref="refPatch" :model="form" :rules="rules" label-width="120px">
  ...
  <template #footer>
    <div style="flex: auto">
      ...
      <el-button type="primary" @click="submitPatch(refPatch)">提交</el-button>
    </div>
  </template>
</el-drawer>
...
const getList = async (ifLoading) => {
  ...
  if(cluster.value !== "" && namespace.value !== "") {
    let response = await axios.get(`/lizardcd/cluster/${cluster.value}/namespace/${namespace.value}/statefulsets`)
    if(response) {
...
const openDeploy = async (row) => {
  ...
  let response = await axios.get(`/lizardcd/cluster/${cluster.value}/namespace/${namespace.value}/statefulsets/${row.name}/pods`)
  ...
  switch(command.action) {
    case "restart": {
      await ElMessageBox.confirm('确定重启？','警告', {
        ...
        await axios.patch(`/lizardcd/cluster/${cluster.value}/namespace/${namespace.value}/statefulsets/${command.row.name}/rollout`)
        ElMessage.success({message: '操作成功'})
      }).catch(() =>{})
      break
    }
    case "setImage": {
      let pods = await axios.get(`/lizardcd/cluster/${cluster.value}/namespace/${namespace.value}/statefulsets/${command.row.name}/pods`)
      let container = pods[0].spec.containers[0].name
      await ElMessageBox.prompt(`请输入容器 ${container} 的镜像`,'提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      }).then(async ({value}) => {
        await axios.patch(`/lizardcd/cluster/${cluster.value}/namespace/${namespace.value}/statefulsets/${command.row.name}?container=${container}&image=${value}`)
        ElMessage.success({message: '操作成功'})
      }).catch(() =>{})
      break
    }
    case "scale": {
      await ElMessageBox.prompt(`请输入副本数`,'提示', {
        ...
        await axios.patch(`/lizardcd/cluster/${cluster.value}/namespace/${namespace.value}/statefulsets/scale`, {
          workloads: [
            {
...
const submitPatch = async (f) => {
  if(!f) return
  await f.validate(async (valid) => {
    if(valid) {
      await axios.patch(`/lizardcd/cluster/${form.value.cluster}/namespace/${form.value.namespace}/statefulsets/${form.value.workload_name}?container=${form.value.container}&image=${form.value.image}`)
...
const submitNew = async (f) => {
  ...
  await f.validate(async (valid) => {
    if(valid) {
      ...
      await axios.patch(`/lizardcd/cluster/${formNew.value.cluster}/namespace/${formNew.value.namespace}/apply/yaml`, params)
      ElMessage.success({message: '发布成功'})
